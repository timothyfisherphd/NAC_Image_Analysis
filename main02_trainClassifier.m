...
Description: This script is used to train a new classifier. The resulting
classifier will be saved with a random unique ID.

Input: Texture Features

Output: Classifier and Performance Metrics (Accuracy, Confusion Matrix, ROC)
...
% set constants
cnst.crossVal = 18;             % number of cross validations, default 10
...
cnst.allFSets = {'all6'}; %{'histogram_lower','histogram_higher','gabor','perceptual','f-lbp','glcm','best5'};  
...
%% Step 1: Iterate through all methods and classifiers, perform all experiments.
...
for currNumClass = [numel(maxClasses)] % first and last element [1,numel(maxClasses)]
    ...
    tic,   [trainedClassifier, validationAccuracy, ConfMat, ROCraw] = trainMyClassifier(source_target_limit_classes,1:maxClass,cnst.crossVal,currClassifier); elapsedTime = toc;
